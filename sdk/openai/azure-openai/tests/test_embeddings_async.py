...
class TestEmbeddingsAsync(AzureRecordedTestCase):
    ...
    async def test_embedding_bad_deployment_name(self, client_async, azure_openai_creds, api_type):
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("api_type", [AZURE])
    async def test_embedding_kw_input(self, client_async, azure_openai_creds, api_type):
    ...
    async def test_embedding(self, client_async, azure_openai_creds, api_type):
        model = azure_openai_creds["embeddings_model"] if api_type == "openai" \
          else azure_openai_creds["embeddings_name"]

        embedding = await client_async.embeddings.create(input="hello world", model=model)
    ...
    async def test_embedding_batched(self, client_async, azure_openai_creds, api_type):
        model = azure_openai_creds["embeddings_model"] if api_type == "openai" \
          else azure_openai_creds["embeddings_name"]
        embedding = await client_async.embeddings.create(input=["hello world", "second input"], model=model)
        assert embedding.object == "list"
        assert embedding.model
        assert embedding.usage.prompt_tokens is not None
        assert embedding.usage.total_tokens is not None
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("api_type", [AZURE, OPENAI])
    async def test_embedding_user(self, client_async, azure_openai_creds, api_type):
        model = azure_openai_creds["embeddings_model"] if api_type == "openai" \
          else azure_openai_creds["embeddings_name"]

        embedding = await client_async.embeddings.create(input="hello world", user="krista", model=model)
        assert embedding.object == "list"
        assert embedding.model
        assert embedding.usage.prompt_tokens is not None
        assert embedding.usage.total_tokens is not None
