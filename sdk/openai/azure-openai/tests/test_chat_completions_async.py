...
class TestChatCompletionsAsync(AzureRecordedTestCase):
    ...
    async def test_chat_completion_user(self, azure_openai_creds, api_type):
        ...
        completion = await openai.ChatCompletion.acreate(messages=messages, user="krista", **kwargs)
    ...
    async def test_chat_completion_logit_bias(self, azure_openai_creds, api_type):
        ...
        completion = await openai.ChatCompletion.acreate(
            messages=messages, logit_bias={17585: -100, 14573: -100}, **kwargs
        )
        assert completion.id
        assert completion.object == "chat.completion"
        assert completion.created
    ...
    async def test_chat_completion_functions(self, azure_openai_creds, api_type):
        ...
        completion = await openai.ChatCompletion.acreate(messages=messages, functions=functions, **kwargs)
        assert completion.id
        assert completion.object == "chat.completion"
        assert completion.created
        assert completion.model
        ...
        function_completion = await openai.ChatCompletion.acreate(messages=messages, functions=functions, **kwargs)
    ...
    async def test_chat_completion_functions_stream(self, azure_openai_creds, api_type):
        ...
        response = await openai.ChatCompletion.acreate(messages=messages, functions=functions, stream=True, **kwargs)
        args = ""
        async for completion in response:
            for c in completion.choices:
                assert c.delta is not None
    ...
    async def test_chat_completion_functions_rai(self, azure_openai_creds, api_type):
        ...
        with pytest.raises(openai.error.InvalidRequestError) as e:
            response = await openai.ChatCompletion.acreate(messages=messages, functions=functions, **kwargs)
